...
(function ($) {
    // 定义 adamsOverload 函数 - 初始化页面功能
    $.adamsOverload = function() {
        // 初始化时间格式化
        if (typeof $.lately === 'function') {
            $.lately({
                target: '.time'
            });
        }
    ...
    $(document).on('click', '.setting_tool a', function (e) {
        if ($(this).is('.back2top')) {
            ...
            if ($(this).is('.socolor') && !_let.st.is('.colors')) {
    ...
    // 处理打赏按钮点击
    $(document).on('click', '.infos .donate > a', function (e) {
        e.preventDefault();
        if ($('.infos').hasClass('donate-close')) {
            $('.infos').removeClass('donate-close');
        } else {
            $('.infos').removeClass('share-close').addClass('donate-close');
        }
    });

    // 处理分享按钮点击
    $(document).on('click', '.infos .share > a', function (e) {
        e.preventDefault();
        if ($('.infos').hasClass('share-close')) {
            $('.infos').removeClass('share-close');
        } else {
            $('.infos').removeClass('donate-close').addClass('share-close');
        }
    });

    // 点击其他地方关闭打赏和分享窗口
    $(document).on('click', function (e) {
        if (!$(e.target).closest('.infos .donate, .infos .share').length) {
            $('.infos').removeClass('donate-close share-close');
        }
    });
